	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			prepare: func(pubkeys []string, signs []string, _ []string, _ map[string][]byte, _ string) []string {
				...
				return append(append([]string{"req", "acl", "acl", "3", nonce}, pubkeys...), signs...)
			},
		},
		{
			...
			prepare: func(pubkeys []string, signs []string, _ []string, _ map[string][]byte, _ string) []string {
				nonce := strconv.Itoa(int(time.Now().Unix() * 1000))
				return append(append([]string{"req", "acl", "acl", "3", nonce}, pubkeys...), signs[1:]...)
			},
		},
		{
			...
			prepare: func(pubkeys []string, signs []string, priv []string, _ map[string][]byte, _ string) []string {
				...
				signs[2] = string(common.Base58EncodedSignature(base58.Decode(priv[2]), hash[:]))
				return append(append([]string{"req", "acl", "acl", "3", nonce}, pubkeys...), signs...)
			},
		},
		{
			...
			prepare: func(pubkeys []string, signs []string, _ []string, _ map[string][]byte, _ string) []string {
				nonce := strconv.Itoa(int(time.Now().Unix() * 1000))
				return append(append([]string{"req", "acl", "acl", "10", nonce}, pubkeys...), signs...)
			},
		},
		{
		...
		},
		{
			...
			prepare: func(pubkeys []string, signs []string, _ []string, state map[string][]byte, addressMulti string) []string {
				...
				nonceB, _ := new(big.Int).SetString(nonce, 10)
				state[keyNonceMulti] = nonceB.Bytes()
				return append(append([]string{"req", "acl", "acl", "3", "1"}, pubkeys...), signs...)
			},
		},
		{
		...
		t.Run(testCase.description, func(t *testing.T) {
			...
			for _, privateKey := range privateKeys {
				signatures = append(signatures, string(common.Base58EncodedSignature(base58.Decode(privateKey), message[:])))
			}
			...
			ccAcl := cc.New()
			args := append(append([]string{"req", "acl", "acl", "3", nonce}, pubKeys...), signatures...)
			if testCase.prepare != nil {
				args = testCase.prepare(pubKeys, signatures, privateKeys, state, addressMulti)
			}

			mockStub.GetFunctionAndParametersReturns(common.FnAddMultisigWithBase58Signature, args)
			...
			require.True(t, proto.Equal(&pb.SignedAddress{
				...
				SignedTx: []string{common.FnAddMultisigWithBase58Signature, "req", "acl", "acl", "3", nonce,
