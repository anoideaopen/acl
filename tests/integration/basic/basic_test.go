...
var _ = Describe("ACL basic tests", func() {
	...
	It("Add user test & check keys test", func() {
		...
		By("checking result")
		ts.Query(cmn.ChannelAcl, cmn.ChannelAcl, FnCheckKeys, user.PublicKeyBase58).CheckResponseWithFunc(aclcmn.CheckKeys(aclcmn.TestAccountNotListed, user))
	})
	...
	It("Change public key with hex encoded key", func() {
		...
		aclclient.ChangePublicKey(network, peer, network.Orderers[0], oldUser, newUser.PublicKeyBase58, "0", "0", ts.Admin())

		By("checking result")
		ts.Query(cmn.ChannelAcl, cmn.ChannelAcl, FnCheckKeys, newUser.PublicKeyBase58).CheckResponseWithFunc(aclcmn.CheckKeys(aclcmn.TestAccountNotListed, oldUser))
	})
	...
	It("Change multisigned user public key", func() {
		...
		ts.ChangeMultisigPublicKey(multisignedUser, oldUser.PublicKeyBase58, newUser.PublicKeyBase58, "reason", "0", ts.Admin())
	...
	It("Check address test", func() {
		...
		By("checking address")
		ts.Query(cmn.ChannelAcl, cmn.ChannelAcl, FnCheckAddress, user.AddressBase58Check).CheckResponseWithFunc(aclcmn.CheckAddress(user))

		By("add user to gray list")
		aclclient.AddToGrayList(network, peer, network.Orderers[0], user)

		By("checking address")
		ts.Query(cmn.ChannelAcl, cmn.ChannelAcl, FnCheckAddress, user.AddressBase58Check).CheckErrorEquals("address " + user.AddressBase58Check + " is graylisted")
	})
	...
	It("Get Addresses test", func() {
		...
		By("checking users")
		ts.Query(cmn.ChannelAcl, cmn.ChannelAcl, FnGetAddresses, "100", "").CheckResponseWithFunc(aclcmn.CheckAddresses(user1, user2, user3))
	})
})
