...
var _ = Describe("ACL emission tests", func() {
	...
	It("Emit transfer test", func() {
		...
		ts.DeployFiat(ts.Admin().AddressBase58Check, ts.FeeSetter().AddressBase58Check, ts.FeeAddressSetter().AddressBase58Check)
		...
		ts.Query(cmn.ChannelFiat, cmn.ChannelFiat,
			FnBalanceOf, user.AddressBase58Check).CheckBalance(emitAmount)
	})
