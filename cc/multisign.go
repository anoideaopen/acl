...
func (c *ACL) AddMultisig(stub shim.ChaincodeStubInterface, args []string) error {
	...
	if err = addMultisig(stub, request); err != nil {
		return fmt.Errorf("failed adding multisig: %w", err)
	}
...
func (c *ACL) ChangeMultisigPublicKey(stub shim.ChaincodeStubInterface, args []string) error { //nolint:funlen,gocyclo,gocognit
	const minArgsCount = 8

	argsNum := len(args)
	if argsNum < minArgsCount {
		return fmt.Errorf("incorrect number of arguments: %d, but this method expects: address, old key, new key, reason, reason ID, nonce, public keys, signatures", argsNum)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to convert reason ID to int, err: %w", err)
	}
	...
	if len(oldKey) == 0 {
		return errors.New("empty old key")
	}
	...
	if err != nil {
		return fmt.Errorf("failed getting public key composite key: %w", err)
	}
	...
	if len(keys) == 0 {
		return fmt.Errorf("no public keys for address %s", multisigAddr)
	}

	pkToAddrCompositeKey, err := compositekey.SignedAddress(stub, string(keys))
	if err != nil {
		return fmt.Errorf("failed getting signed address composite key: %w", err)
	}
	...
	if len(signedAddrBytes) == 0 {
		return fmt.Errorf("no SignedAddress msg for address %s", multisigAddr)
	}
	...
	if err = helpers.CheckKeysArr(strKeys); err != nil {
		return fmt.Errorf("failed checking keys array: %w", err)
	}
	hashedHexKeys, err := helpers.KeyStringToSortedHashedHex(strKeys)
	if err != nil {
		return fmt.Errorf("failed converting keys to hex: %w", err)
	}
	...
	if err = stub.PutState(newPkToAddrCompositeKey, addrChangeMsg); err != nil {
		return fmt.Errorf("failed putting signed address to state: %w", err)
	}

	// set new address -> key mapping
	if err = stub.PutState(addrToPkCompositeKey, []byte(hashedHexKeys)); err != nil {
		return fmt.Errorf("failed putting public keys to state: %w", err)
	}
