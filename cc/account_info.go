...
func (c *ACL) SetAccountInfo(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	const argsLen = 4
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get account info composite key: %w", err)
	}

	if _, err = checkIfAccountInfoExistsAndGetData(stub, cKeyInfo, addrEncoded); err != nil {
		return nil, fmt.Errorf("failed checking if account info exists: %w", err)
	}
	...
	grayListed := args[2]
	if len(grayListed) == 0 {
		return nil, errors.New("grayList attribute is not set")
	}

	isGrayListed, err := strconv.ParseBool(grayListed)
	if err != nil {
		return nil, fmt.Errorf("failed to parse graylist attribute: %w", err)
	}

	blacklisted := args[3]
	if len(blacklisted) == 0 {
		return nil, errors.New("blacklisted attribute is not set")
	}

	isBlacklisted, err := strconv.ParseBool(blacklisted)
	if err != nil {
		return nil, fmt.Errorf("failed to parse blacklist attribute: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal account info message: %w", err)
	}
	...
	if err = stub.PutState(cKey, infoMsg); err != nil {
		return nil, fmt.Errorf("failed to put account info message into state: %w", err)
	}
